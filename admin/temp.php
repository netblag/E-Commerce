    ...
    <?php 
        if (isset($_POST['login'])) {
            ...
            if (empty($_POST['userEmail']) || empty($_POST['password'])) {
                echo '<div class="alert alert-danger">تمامی فیلدها باید پر شوند.</div>';
                die();
            } else {
                ...
                $result = mysqli_query($conn, $sql) or die("خطا در کوئری.");
            
                if (mysqli_num_rows($result) > 0) {
                    $_SESSION['logged-in'] = '1';
                    header("Location: ./post.php");
                    } else {
                    echo '<div class="alert alert-danger">ایمیل و رمز عبور مطابقت ندارند.</div>';
                    }
                }
            }
    ?>
