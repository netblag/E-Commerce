...
if(isset($_POST['login'])){
    ...
    if(empty($_POST['email'])){
           echo "<h4 id='error_login'>ایمیل را وارد کنید</h4>";
    }

    if(empty($_POST['pwd'])){
        echo "<h4 id='error_login'>رمز عبور را وارد کنید</h4>";
 }
...
if($result->num_rows==1){ //if any one data found go inside it
    ...
    header("location:profile.php?id={$_SESSION['id']}");
            ...
            exit;}else{
                echo "<h4 id='error_login'>رمز عبور نادرست است</h4>";//as user get inside if statem if userEmail matched
            }


}else{
    if($_POST['email']){ //it means it will run if email field is filled
    echo "<h4 id='error_login'>(موجود نیست) لطفاً ابتدا ثبت‌نام کنید</h4>";
    }
}
